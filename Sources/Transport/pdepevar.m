function varargout = pdepevar(m,pde,ic,bc,xmesh,t,options, Resolt_bolean,varargin)
...
%   $Revision: 1.8.4.9 $  $Date: 2011/05/17 02:23:22 $
...
varargout{1}=pdeodes(t(1),y0); % Look at

if Resolt_bolean == true
...
d_sol=1;   % case solution
varargout{1+d_sol} = sol;
if hasEvents % [sol,t,sole,te,ie] = pdepe(...)    
  ...
  varargout{3+d_sol} = sole;
  varargout{4+d_sol} = te(:);
  varargout{5+d_sol} = ie(:);
end
end
  ...
  %   for the ODE system. 
    ...
    u = reshape(y,npde,nx);
    up = zeros(npde,nx);
    [U,Ux] = pdentrp(m,xmesh(1),u(:,1),xmesh(2),u(:,2),xi(1));
    ...
    if xmesh(1) == 0 && m > 0   %  Singular
      ...
      for j = 1:npde
        if qL(j) == 0
          ...
          up(j,1) = pL(j) + (qL(j) / xmesh(1)^m) * ...
                    (xim(1) * fL(j) + zxmp1(1) * sL(j));
          denom = (qL(j) / xmesh(1)^m) * zxmp1(1) * cL(j);
          if denom ~= 0
            up(j,1) = up(j,1) / denom;
          end
        end
    ...
    %  Interior points
    for i = 2:nx-1
      [U,Ux] = pdentrp(m,xmesh(i),u(:,i),xmesh(i+1),u(:,i+1),xi(i));
    ...
    for j = 1:npde
      if qR(j) == 0
        ...
        up(j,nx) = pR(j) + (qR(j) / xmesh(nx)^m) * ...
            (xim(nx-1) * fL(j) - xzmp1(nx) * sL(j));
        denom = -(qR(j) / xmesh(nx)^m) * xzmp1(nx) * cL(j);
        if denom ~= 0
